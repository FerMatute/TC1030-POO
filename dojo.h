 ...
 */
 ...
 void Dojo::lucha_Card_Jitsu(){
	...
	*/
	...
	elem = user.cartas_U[choice - 1]->get_tipo();
	level = user.cartas_U[choice - 1]->get_nivel();
	cout<<"\nSeleccion usuario: "<<user.cartas_U[choice - 1]->to_string();
	...
	cout<<"\nSeleccion sensei: "<<sensei.cartas_U[choiceS]->to_string();
	//Evaluamos
...
//Cuando se selecciona la carta de Nieve
    ...
    cin >> ch;
    if(ch == 'n' || ch == 'N'){ // Comilla simple para char y doble string
    	break;
	}
    system("CLS");
    user.muestra_cartasU();
	}
